...
mod_model_ntt_ui <- function(id) {
  ...
  glassCard(
    ...
    toolbar = tagList(
      glassDropdown(
        inputId = ns("model_variant_select"),
        choices = c(
          "NTT" = "ntt",
          "NNN" = "nnn",
          "NTN" = "ntn",
...
#' Model 1 (NTT/NNN/NTN/NNT) Module Server
#'
#' @param id Namespace ID for the module
...
mod_model_ntt_server <- function(id, app_state) {
  moduleServer(id, function(input, output, session) {
    # =====================================================================
    # Helper: map variant key to appstate field names
    # =====================================================================

    get_variant <- function() {
      input$model_variant_select %||% "ntt"
    }

    model_field <- function(suffix) {
      paste0("model_", get_variant(), "_", suffix)
    }

    compiled_field <- function() {
      paste0("compiled_model_", get_variant())
    }

    get_results <- function() {
      app_state[[model_field("results")]]
    }

    get_status <- function() {
      app_state[[model_field("status")]] %||% "not_run"
    }
    ...
    # =====================================================================
    # Update Run Button Label on Variant Change
    # =====================================================================

    observeEvent(input$model_variant_select, {
      updateGlassButton(
        session,
        "run_btn",
        label = paste("Run", model_label(), "Model")
      )
    }, ignoreInit = TRUE)
    ...
    observeEvent(input$run_btn,
      {
        variant <- get_variant()
        variant_upper <- model_label()
        results_key <- model_field("results")
        status_key <- model_field("status")
        error_key <- model_field("error")

        tryCatch(
          {
            app_state[[results_key]] <- run_bayesian_analysis(
              model = app_state[[compiled_field()]],
              model_name = paste(variant_upper, "Model"),
              app_state = app_state
            )

            # Determine status based on warnings
            if (length(app_state[[results_key]]$advice) > 0) {
              app_state[[status_key]] <- "warning"
            } else {
              app_state[[status_key]] <- "success"
            }
          },
          error = function(e) {
            app_state[[status_key]] <- "error"
            app_state[[error_key]] <- e$message
            showGlassToast(
              message = paste(variant_upper, "model failed:", e$message),
    ...
    output$overview_diagnostics <- renderUI({
      ...
      if (is.null(results)) {
        return(tags$div(
          ...
          tags$span(
            class = "awaiting-subtext",
            paste0("Click \"Run ", model_label(), " Model\" to start the analysis")
          )
        ))
      }
    ...
    shiny::observeEvent(list(get_results()), {
      results <- get_results()
      shiny::req(results)
      payload <- prepare_subject_cvi_d3(results$results)
    ...
    shiny::observeEvent(list(get_results()), {
      results <- get_results()
      shiny::req(results)
      payload <- prepare_cvi_vs_concentration_d3(
        results$results
      )
    ...
    shiny::observeEvent(list(get_results()), {
      results <- get_results()
      shiny::req(results)
      payload <- prepare_cvi_vs_rcv_d3(results$results)
    ...
    update_trace_posterior_plot <- function() {
      results <- get_results()
      plotting_data <- results$additional_results$trace_and_posterior_plotting_data
    ...
    # Re-render when model results change (due to run or variant switch)
    shiny::observeEvent(list(get_results()), {
      results <- get_results()
      shiny::req(results)
